...
fns.findGenericById = (idArray, collection) => {

    return Promise.all(idArray.map(sid => util.safeObjectId(sid)))
        .then(oIds => {
...
fns.getPushTokenFromUser = (userIds) => {

    return database.collection(COLLECTION_DEVICE)
        .find({user_id: {$in: userIds}}, {pushToken: 1, _id:0, type:1}).toArray();
};
