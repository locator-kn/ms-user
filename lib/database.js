...
fns.findUserByFbId = user =>{
    return database.collection(COLLECTION_USER).find(user);
};
...
fns.updateUserWithTempPassword = (mail, hash) => {
    return database.collection(COLLECTION_USER).updateOne(
        {
            mail: mail
        },
        {
...
fns.deleteTempPw = (userId) => {
    util.safeObjectId(userId, 'user_id')
        .then(oId => {
            return database.collection(COLLECTION_USER)
                .updateOne(
                    {_id: oId},
