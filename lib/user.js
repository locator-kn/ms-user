...
fns.fbLogin = (message,next) =>{
    ...
    Joi.validate(message.data,validation.fbLogin, (err,user) =>{
        if (err) {
            return util.validationError(err, 'Facebook login service');
        }

        let deviceId = user.requesting_device_id;
        delete user.requesting_device_id;

        db.findUserByFbId(message.data.id)
            .then(dbUser => {
                if(dbUser){
                    //TODO: i know this user, give him back
        ...
        .then(()=>{
                //TODO: activate device id)
                console.log(user);//jshint
            });


        console.log(message,next);//hello jshint ;)
        return;
